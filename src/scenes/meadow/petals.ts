...
// Light -> saturated gradient pairs. Yellow weighted heaviest per feedback.
const PETAL_COLORS: { from: string; to: string; weight: number }[] = [
  { from: '#fff2c2', to: '#e6b23f', weight: 4 }, // yellow — most common
  { from: '#ffd9e6', to: '#ffb6c9', weight: 2 }, // pink
  { from: '#e3c9f2', to: '#b985d9', weight: 2 }, // purple
  { from: '#ffc9bd', to: '#dd6a56', weight: 2 }, // red
]
const PETAL_COLOR_TOTAL_WEIGHT = PETAL_COLORS.reduce((sum, c) => sum + c.weight, 0)

function pickPetalColor(rand: () => number) {
  let roll = rand() * PETAL_COLOR_TOTAL_WEIGHT
  for (const color of PETAL_COLORS) {
    if (roll < color.weight) return color
    roll -= color.weight
  }
...
export function generatePetals(count = 5, seed = 11): PetalDef[] {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const color = pickPetalColor(rand)
